...
<body>
	<h2>Welcome to hello application</h2>
	...
	<h3>Routing HTTP and WebSocket traffic to your application based on the path of the URL</h3>
		...
		<pre>git azure app --setup hello --disablePathRouting
git add .
git commit
git push</pre>
		<p>Similarly, you can re-enable URL path routing with:
		<pre>git azure app --setup hello --enablePathRouting
...</pre>
	<h3>Routing HTTP and WebSocket traffic to your application based on host name of the URL</h3>
		...
		<pre>git azure app --setup hello --host myhostname.com
git add apps/hello/package.json
git commit
git push</pre>
	...
	<h3>Enabling SSL</h3>
		...
		<pre>git azure app --setup hello --host myservice.com --ssl allowed --certFile myCertificate.pem --keyFile myKey.pem</pre>
		<p>The myCertificate.pem and myKey.pem files must contain the PKCS#7 encoded (PEM) X.509 certificate 
 		...
 		<p>Alternatively, you can also quickly generate a self-siged X.509 certificate corresponding to your 
 			host name with:
 		<pre>git azure app --setup hello --host myservice.com --ssl allowed --generateX509</pre>
